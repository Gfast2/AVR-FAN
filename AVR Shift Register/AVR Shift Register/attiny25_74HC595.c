...
int main(void)
{
	...
	OCR1C = 127; // setting the PWM speed. In this case, it is working @100KHz
	TCCR1 |= (1 << PWM1A) | (1 << COM1A0) /*| (1 << CS11)*/ | (1 << CS10); // PWM mode + clear OC1A output line + CK/2. COM1A0!! Page:87
...
ISR(ADC_vect)
{
	...
	dutyCycle = voltage_reading / 8; // voltage_reading: 0 ~ 1023, dutyCycle: 0 ~ 255
	ADCSRA |= 1<<ADSC;
}
